...
/*
 * Vanilla module adaptation of React Bits' Liquid Ether component.
 * Original source: https://github.com/DavidHDev/react-bits
 * License retained at ../vendor/react-bits-liquid-ether/LICENSE.md
 */
export function initLiquidEther({
  ...
  const mountRef = { current: container };
  const webglRef = { current: null };
  const resizeObserverRef = { current: null };
  const rafRef = { current: null };
  const intersectionObserverRef = { current: null };
  const isVisibleRef = { current: true };
  const resizeRafRef = { current: null };
...
`;
    ...
    if (getComputedStyle(mount).overflow === 'visible') mount.style.overflow = 'hidden';
    ...
    return {
      destroy,
      resize: () => webglRef.current?.resize()
    };
}
