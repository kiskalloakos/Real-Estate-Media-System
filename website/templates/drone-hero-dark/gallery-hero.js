...
function renderPricesSection(config) {
  ...
  if (!prices?.rates?.length) return "";
  const visibleNotes = prices.notes || [];

  return `
    <section class="gh-prices" id="prices" aria-label="${escapeHtml(prices.title)}">
      <div class="gh-prices-shell">
        <div class="gh-price-card" aria-label="${escapeHtml(prices.cardLabel)}">
          <div class="gh-price-content">
            ...
            <a class="gh-price-action" href="${escapeHtml(config.whatsappUrl)}" target="_blank" rel="noopener">
...
function renderReviewBadge(config, className = "", suffix = "") {
  ...
  return `
    <a class="gh-review-badge ${escapeHtml(className)}" href="${escapeHtml(config.googleMapsUrl)}" target="_blank" rel="noopener" aria-label="${escapeHtml(config.ui.reviewsAria)}">
